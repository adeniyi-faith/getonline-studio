...
class AI_Chat_Admin {
    ...
    public static function register_settings() {
        // General
        register_setting('ai_chat_settings_group', 'ai_chat_enabled');
        register_setting('ai_chat_settings_group', 'ai_chat_bot_name');
        register_setting('ai_chat_settings_group', 'ai_chat_default_provider');
        register_setting('ai_chat_settings_group', 'ai_chat_provider_order');
        register_setting('ai_chat_settings_group', 'ai_chat_api_openai');
        register_setting('ai_chat_settings_group', 'ai_chat_api_claude');
        register_setting('ai_chat_settings_group', 'ai_chat_api_gemini');
        register_setting('ai_chat_settings_group', 'ai_chat_api_cohere');
        register_setting('ai_chat_settings_group', 'ai_chat_api_mistral');
    ...
    public static function page_dashboard() {
        ...
        <div class="wrap">
            ...
            echo "<ul><li>Total conversations: <strong>".intval($total)."</strong></li><li>Total messages: <strong>".intval($messages)."</strong></li><li>Avg messages per conversation: <strong>".round(floatval($avg),2)."</strong></li></ul>";
            ?>
    ...
    public static function page_settings() {
        if (!current_user_can('manage_options')) wp_die('Unauthorized');
        if(isset($_POST) && isset($_POST['ai_chat_enabled'])) {
            // handled by settings API
        }
        ?>
        <div class="wrap">
            ...
            <form method="post" action="options.php">
                ...
                <table class="form-table">
                    <tr><th>Enable chatbot</th><td><input type="checkbox" name="ai_chat_enabled" value="1" <?php checked(get_option('ai_chat_enabled','1'),'1'); ?> /></td></tr>
                    <tr><th>Bot name</th><td><input type="text" name="ai_chat_bot_name" value="<?php echo esc_attr(get_option('ai_chat_bot_name')); ?>" /></td></tr>
                    <tr><th>Provider order (fallback)</th><td><input type="text" name="ai_chat_provider_order" value="<?php echo esc_attr(get_option('ai_chat_provider_order')); ?>" style="width:60%"/><p class="description">Comma-separated provider keys, e.g. openai,claude,gemini,cohere,mistral</p></td></tr>
                    <tr><th>Default provider</th><td><select name="ai_chat_default_provider">
                        <?php $def = get_option('ai_chat_default_provider','openai'); $providers = ['openai'=>'OpenAI','claude'=>'Anthropic Claude','gemini'=>'Google Gemini','cohere'=>'Cohere','mistral'=>'Mistral']; foreach($providers as $k=>$v){ echo '<option value="'.esc_attr($k).'" '.selected($def,$k,false).'>'.esc_html($v).'</option>'; } ?>
                    </select></td></tr>
                    <tr><th>OpenAI API key</th><td><input type="password" name="ai_chat_api_openai" value="<?php echo esc_attr(get_option('ai_chat_api_openai')); ?>" style="width:60%"/></td></tr>
                    <tr><th>Claude API key</th><td><input type="password" name="ai_chat_api_claude" value="<?php echo esc_attr(get_option('ai_chat_api_claude')); ?>" style="width:60%"/></td></tr>
                    <tr><th>Gemini API key</th><td><input type="password" name="ai_chat_api_gemini" value="<?php echo esc_attr(get_option('ai_chat_api_gemini')); ?>" style="width:60%"/></td></tr>
                    <tr><th>Cohere API key</th><td><input type="password" name="ai_chat_api_cohere" value="<?php echo esc_attr(get_option('ai_chat_api_cohere')); ?>" style="width:60%"/></td></tr>
                    <tr><th>Mistral API key</th><td><input type="password" name="ai_chat_api_mistral" value="<?php echo esc_attr(get_option('ai_chat_api_mistral')); ?>" style="width:60%"/></td></tr>
    ...
    public static function page_widget() {
        ...
        <div class="wrap">
            ...
            <h2>Preview</h2>
            <div id="ai-chat-preview" style="min-height:220px;border:1px solid #ddd;padding:10px;border-radius:8px;">
                <div style="padding:10px;background:#fff;border-radius:6px;box-shadow:0 2px 6px rgba(0,0,0,.06);">
                    <strong><?php echo esc_html(get_option('ai_chat_bot_name','Assistant')); ?></strong>
                    <p><?php echo esc_html(get_option('ai_chat_welcome')); ?></p>
                </div>
            </div>
            <h2>Custom CSS</h2>
            <p>To override widget styles, add custom CSS to your theme or a custom CSS plugin.</p>
        </div>
        <?php
    }
    ...
    public static function page_analytics() {
        ...
        <div class="wrap">
            ...
            <h2>Top Questions</h2>
            <?php
            // improved top questions aggregation
            $top = $wpdb->get_results("SELECT content, COUNT(*) as c FROM {$wpdb->prefix}ai_chat_messages WHERE role='user' GROUP BY content ORDER BY c DESC LIMIT 50");
            if($top){
                echo '<table class="widefat"><thead><tr><th>Question (truncated)</th><th>Count</th></tr></thead><tbody>';
                foreach($top as $t){
                    $q = esc_html(mb_strimwidth($t->content,0,120,'...'));
                    echo '<tr><td>'.$q.'</td><td>'.intval($t->c).'</td></tr>';
                }
                echo '</tbody></table>';
            } else {
                echo '<p>No user messages yet.</p>';
            }
            ?>
        </div>
        <?php
