...
class Permissions {
	...
	public function set_permission( $file = [], $permission_role = 'reader'  ) {
		...
		if ( $manage_permissions && $file_permissions['canShare'] ) {
			...
			try {
				...
				// If the permission is set to 'anyone', insert a log
				if (igd_fs()->can_use_premium_code__premium_only() && 'anyone' === $permission_type ) {

					if ( ! class_exists( 'IGD\Statistics' ) ) {
						include_once IGD_INCLUDES . "/class-statistics__premium_only.php";
					}

					Statistics::instance()->insert_log( [
						'type'       => 'shared',
	...
	public function share_file( $file, $user_data ) {
		...
		if ( empty( $file['accountId'] ) || empty( $file['id'] ) || empty( $user_data['email'] ) ) {
			return false;
		}
