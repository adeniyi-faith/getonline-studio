...
function csco_mb_layout_options() {
	...
	$function( 'csco_mb_layout_options', esc_html__( 'Layout Options', 'overflow' ), 'csco_mb_layout_options_callback', array( 'post', 'page', 'product' ), 'side' );
}
...
function csco_mb_layout_options_callback( $post ) {
	...
	?>
		<h4><?php esc_html_e( 'Sidebar', 'overflow' ); ?></h4>
		<select name="csco_singular_sidebar" id="csco_singular_sidebar" style="box-sizing: border-box;" class="regular-text">
			<option value="default" <?php selected( 'default', $sidebar ); ?>> <?php esc_html_e( 'Default', 'overflow' ); ?></option>
			<option value="right" <?php selected( 'right', $sidebar ); ?>> <?php esc_html_e( 'Right Sidebar', 'overflow' ); ?></option>
			<option value="left" <?php selected( 'left', $sidebar ); ?>> <?php esc_html_e( 'Left Sidebar', 'overflow' ); ?></option>
			<option value="disabled" <?php selected( 'disabled', $sidebar ); ?>> <?php esc_html_e( 'No Sidebar', 'overflow' ); ?></option>
		</select>
		...
		<?php if ( ! in_array( (string) $post->ID, $page_static, true ) || 'posts' === get_option( 'show_on_front', 'posts' ) ) { ?>
			...
			<?php if ( 'post' === $post->post_type || 'page' === $post->post_type ) { ?>
				<h4><?php esc_html_e( 'Page Header Type', 'overflow' ); ?></h4>
				<select name="csco_page_header_type" id="csco_page_header_type" style="box-sizing: border-box;" class="regular-text">
					<option value="default" <?php selected( 'default', $page_header_type ); ?>> <?php esc_html_e( 'Default', 'overflow' ); ?></option>
					<option value="standard" <?php selected( 'standard', $page_header_type ); ?>> <?php esc_html_e( 'Standard', 'overflow' ); ?></option>
					<option value="large" <?php selected( 'large', $page_header_type ); ?>> <?php esc_html_e( 'Large', 'overflow' ); ?></option>
					<option value="title" <?php selected( 'title', $page_header_type ); ?>> <?php esc_html_e( 'Page Title Only', 'overflow' ); ?></option>
					<option value="none" <?php selected( 'none', $page_header_type ); ?>> <?php esc_html_e( 'None', 'overflow' ); ?></option>
				</select>
			<?php } ?>

			<?php if ( 'post' === $post->post_type ) { ?>
				<h4><?php esc_html_e( 'Auto Load Next Post', 'overflow' ); ?></h4>
				<select name="csco_page_load_nextpost" id="csco_page_load_nextpost" style="box-sizing: border-box;" class="regular-text">
					<option value="default" <?php selected( 'default', $page_load_nextpost ); ?>> <?php esc_html_e( 'Default', 'overflow' ); ?></option>
					<option value="enabled" <?php selected( 'enabled', $page_load_nextpost ); ?>> <?php esc_html_e( 'Enabled', 'overflow' ); ?></option>
					<option value="disabled" <?php selected( 'disabled', $page_load_nextpost ); ?>> <?php esc_html_e( 'Disabled', 'overflow' ); ?></option>
				</select>
			<?php } ?>
...
function csco_meta_boxe_video_options() {
	...
	$function( 'csco_mb_video_background', esc_html__( 'Video Background', 'overflow' ), 'csco_mb_video_options_callback', array( 'post', 'page' ), 'side' );
}
...
function csco_mb_video_options_callback( $post ) {
	...
	?>
		...
		<label><input type="checkbox" id="csco_post_video_location" name="csco_post_video_location[]" value="large-header" <?php checked( in_array( 'large-header', $post_video_location, true ) ); ?>><?php esc_html_e( 'Large Header', 'overflow' ); ?></label><br>
		<?php if ( 'post' === $post->post_type ) { ?>
			<label><input type="checkbox" id="csco_post_video_location" name="csco_post_video_location[]" value="archive" <?php checked( in_array( 'archive', $post_video_location, true ) ); ?>><?php esc_html_e( 'Post Archives', 'overflow' ); ?></label><br>
			<label><input type="checkbox" id="csco_post_video_location" name="csco_post_video_location[]" value="featured" <?php checked( in_array( 'featured', $post_video_location, true ) ); ?>><?php esc_html_e( 'Featured Posts', 'overflow' ); ?></label><br>
			<label><input type="checkbox" id="csco_post_video_location" name="csco_post_video_location[]" value="inverse" <?php checked( in_array( 'inverse', $post_video_location, true ) ); ?>><?php esc_html_e( 'Featured Posts Widget', 'overflow' ); ?></label><br>
			<label><input type="checkbox" id="csco_post_video_location" name="csco_post_video_location[]" value="footer" <?php checked( in_array( 'footer', $post_video_location, true ) ); ?>><?php esc_html_e( 'Footer Posts', 'overflow' ); ?></label>
		<?php } ?>
		<!-- YouTube URL -->
